...
public class DurationSelectForJailGui extends SimpleGui {
    ...
    public DurationSelectForJailGui(ServerPlayer staff, ServerPlayer target, JailZone zone, SimpleGui parent) {
        super(MenuType.GENERIC_9x2, staff, false);
        this.staff  = staff;
        this.target = target;
        this.zone   = zone;
        this.parent = parent;
        setTitle(Component.literal("§8» §6Duración del jail en §f" + zone.name));
        build();
    }
        ...
        for (int i = 0; i < DURATIONS.length; i++) {
            ...
            setSlot(i, new GuiElementBuilder(selected ? Items.LIME_CONCRETE : Items.GRAY_CONCRETE)
                ...
                .setCallback((idx, type, a, gui) -> {
                    ...
                    for (int s = 0; s < getSize(); s++) setSlot(s, new GuiElementBuilder(net.minecraft.world.item.Items.AIR).build());
                    build();
                })
                .build());
        }
